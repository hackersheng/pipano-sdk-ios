...
- (void)viewDidLoad
{
    ...
    NSString* versionStr = @"Ver:";
    versionStr = [versionStr stringByAppendingString: [PiPano getVersion]];
    [versionLabel setText:versionStr];
    
    //把cameraView装入父view
    piCameraView.frame = CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height);
    superPICameraView.frame = CGRectMake(0, 0, [[UIScreen mainScreen] bounds].size.width, [[UIScreen mainScreen] bounds].size.height);
...
{
    [PiPano getVideoProgress:^(float process){
